...
- 签到结果 Telegram 推送通知
- 今日领取的铜币/银币/金币数量统计
- GitHub Actions 定时执行，10:00~12:00 随机签到，免服务器
...
默认每天北京时间 **10:00~12:00 之间随机**执行（cron 触发后随机延迟 0~2 小时）。

编辑 `.github/workflows/v2ex-checkin.yml`：

```yaml
schedule:
  # cron 触发时间（UTC），北京 10:00 = UTC 02:00
  - cron: '0 2 * * *'

# 随机延迟 0~7200 秒（0~2 小时），实际签到落在 10:00~12:00 之间
DELAY=$((RANDOM % 7200))
```
